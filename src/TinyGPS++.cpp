...
#define _GPGSTterm "GPGST"
#define _GNRMCterm "GNRMC"
#define _GNGGAterm "GNGGA"
#define _GNGSTterm "GNGST"
...
bool TinyGPSPlus::encode(char c) {
  ...
  switch (c) {
    ...
    default:  // ordinary characters
      if (curTermOffset < sizeof(term) - 1) term[curTermOffset++] = c;
...
bool TinyGPSPlus::endOfTermHandler() {
  ...
  if (curTermNumber == 0) {
    ...
    else if (!strcmp(term, _GPGSTterm) || !strcmp(term, _GNGSTterm))
  ...
  if (curSentenceType != GPS_SENTENCE_OTHER && term[0])
    switch (COMBINE(curSentenceType, curTermNumber)) {
      ...
      case COMBINE(GPS_SENTENCE_GPGGA, 9):  // Altitude (GPGGA)
        altitude.set(term);
        break;
      case COMBINE(GPS_SENTENCE_GPGST, 2):  // Fix data (GPGST)
        if (term[0] >= '0') sentenceHasFix = true;
      case COMBINE(GPS_SENTENCE_GPGST, 6):  // Std. dev. of lat error (GPGST)
        stdLat.set(term);
        break;
      case COMBINE(GPS_SENTENCE_GPGST, 7):  // Std. dev. of lon error (GPGST)
        stdLon.set(term);
        break;
    }
